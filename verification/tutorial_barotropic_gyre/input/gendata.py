...
def gendata():
  ...
  h[-1,:]=0;
  h[:,-1]=0;
  fid=open('topog.box','wb'); h.tofile(fid,"",ofmt); fid.close();

  # Generate zonal wind-stress file that is sin with period of dmain north-south extent
  # and maximum of 0.1 Nm^{2}
  tauMax=0.1;
  x=(np.arange(1,nx+1)-0.5)/(nx*1.-1.) # nx-1 accounts for a solid wall
  y=(np.arange(1,ny+1)-0.5)/(ny*1.-1.) # ny-1 accounts for a solid wall
  X,Y=np.meshgrid(x,y);
  tau=tauMax*np.sin(np.pi*Y);
  fid=open('windx.sin_y','wb'); tau.tofile(fid,"",ofmt); fid.close();
